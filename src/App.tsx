...
const dabeeoMaps = new Maps(); // 1번만 호출
...
function App() {
    ...
    const addMap = useCallback(async (parent: HTMLElement) => {
        ...
        const mapOption = {};
        const dabeeoMap = await dabeeoMaps.showMap(mapContainer, mapOption, mapData);
        setMap(dabeeoMap);
        setLoading(false);
    ...
    return (
        <div className="App">
            ...
            {count === 2 ? <Map map={map} addMap={addMap} removeMap={removeMap} setLoading={setLoading} /> : <Main count={count} />}
        </div>
    );
}
